...
| 비서 | 직책 | 역할 | 모델 |
|------|------|------|------|
| 🔍 **CHRIS** | Senior Script Analyst | 구조 분석 · 15-Beat Sheet · 서사 동력 · 시각화 · 장르 정밀 진단축 · 보호 자산 지정 · 오프닝 기법 역추적 | Opus |
| 🧹 **SHIHO** | Script Doctor | 시퀀스 워싱 · 문제 유형 진단 · 대사축 분석 · 처방 균형 게이트 · 각색 제안 · 오프닝 교정 처방 | Opus |
| ✍️ **MOON** | Senior Screenwriter | 수정씬 6개 + 추가씬 4개 · 표준 각본 형식 리라이팅 · 작가 의도 존중 · 보호 자산 보존 | Opus |
...
| **8-D** | **장르 정밀 진단축** — 장르 프로파일 기반 하위축(G1~G6) · 러닝개그 생애주기 | CHRIS |
| **8-E** | **보호 자산** — 수정·삭제 금지 대상 지정 | CHRIS |
| **9-A** | **처방 균형** — 장르 처방 비율 게이트 · 장르 정밀축 처방 | SHIHO |
| 9 | **시퀀스 워싱** — 최대 10개 시퀀스 진단/처방 카드 | SHIHO |
| 10 | **대사 워싱** — 3축(+장르 확장축 ④⑤⑥) 점수 + Before/After 개선 제안 | SHIHO |
...
| 장르 기준값 | 외부 JSON 프로파일 (`genre_profiles/`) |
...
- 📊 **긴장도 곡선** — 씬별 텐션 시각화
- 🎭 **대사 3축 + 장르 확장축** — 캐릭터 적합성 · 서브텍스트 · 행동/감정/관계 (+ 로코: 코믹 타이밍 · 코믹 구체성 · 지위 관계)
...
```
RewriteEngine/
├── main.py              # Streamlit 웹앱 (v2.3)
├── prompt.py            # 3-agent 프롬프트 빌더 + OPENING MASTERY + 장르 프로파일 로더
├── writer_link.py       # Writer Engine 세션 JSON 파서 (보조 참조 데이터)
├── gen_docx.js          # DOCX 생성 (Node.js + docx-js)
├── genre_profiles/      # 장르 정밀 진단 프로파일 (외부 JSON)
│   ├── romantic_comedy.json
│   └── README.md
...
| `build_rewrite_prompt()` | MOON 프롬프트 빌더 (OPENING RX 최상위 규칙 + 보호 자산) |
| `load_genre_profiles()` | `genre_profiles/*.json` 로더 (캐시 · 폴백 안전) |
| `resolve_genre_profile()` | 장르 문자열 → 프로파일 해석 (alias 최장일치) |
| `check_prescription_balance()` | 처방 균형 게이트 서버 측 재계산 |
...
- **장르 프로파일 외부화** — 기준값을 코드에서 분리해 `genre_profiles/*.json`으로 이관. 코드 수정 없이 장르 추가 가능
- **CHRIS `genre` 축 세분화** — 로맨틱 코미디 하위 진단축 G1~G6 신설
  - G1 웃음 밀도 / 무웃음 구간 · G2 3막 유머 유지율 · G3 코미디 셋피스 배치
  - G4 웃음 출처 분포(C1~C6) · G5 러닝개그 생애주기 · G6 주인공 코믹 능동성
  - 러닝개그 회수값 일관성 검사 (설치 252 vs 회수 250 유형 적발)
- **처방 균형 게이트** — 장르 처방이 하한(로코 30%) 미달이면 처방 1회 자동 재생성
- **삭제 처방 대체 지정 의무화** — `replace_with` 없는 삭제는 무효 처방으로 간주
- **보호 자산(`protected_assets`)** — 잘 작동하는 웃음을 수정 금지 대상으로 지정, MOON·Revise에 전달
- **SHIHO 대사축 확장** — ④ 코믹 타이밍 · ⑤ 코믹 구체성 · ⑥ 지위 관계
- **폴백 경고** — 프로파일 조회 실패 시 조용히 넘어가지 않고 경고 출력. 모든 산출물에 `profile_id`/`source` 명시

**엔진 연계 · 작업 편의 (내부용)**

- **Writer Engine 참조 JSON 입력** — 비트별 `GENRE_BOOSTER_CHECK`를 CHRIS 진단의 보조 근거로 활용.
  설계 단계와 원고의 낙차 자체를 진단 대상으로 삼는다. Writer 측 장르 폴백 버그 자동 감지
- **중간 저장 / 복원** — 진행 단계까지의 결과 + 시나리오 본문을 한 파일로 저장, API 재호출 없이 이어서 실행

**버그 수정**

- 장르 표기가 `로맨스/코미디`, `멜로/코미디` 등 슬래시 형태일 때 로코로 인식되지 않던 문제 수정
- 대사축 번호 충돌 정리 — 기존 "④ 개선 제안"의 번호 제거, ④⑤⑥을 장르 확장축에 배정
